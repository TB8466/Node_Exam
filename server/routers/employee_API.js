...
router.post("/api/employee", (req, res) => {
    ...
    const VALUES = [
      [req.body.name || "Name N/A"],[req.body.age || 0],[req.body.position || "Name N/A"],[req.body.wage || 0]
    ];
    db.query("INSERT INTO employees (name,age,position,wage) VALUES (?,?,?,?)", VALUES, (error,result) =>{
        if (error) throw error;
        res.send({ Result : result })
    });
  });
...
router.put("/api/employee", (req, res) => {
  const VALUES = [
    [req.body.name || "Name N/A"],[req.body.age || 0],[req.body.position || "Name N/A"],[req.body.wage || 0],[req.body.id]
  ];
  db.query("UPDATE employees SET name = ?, age = ?, position = ?, wage = ? WHERE id = ?",VALUES, (error,result) => {
    if (error) throw error;
    res.send({ Result : result })
  });
});
